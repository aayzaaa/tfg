...
from tictactoe.TicTacToeGame import TicTacToeGame  # Specify the Game
from tictactoe.tensorflow.NNet import NNetWrapper as nn  # Specify the Neural Network
from utils import *
...
args = dotdict({
    ...
    'numEps': 10,  # Number of self-play games to play during a each iteration.
    'tempThreshold': 4,  # Temperature threshold. Controls when to start playing only the best move.
    ...
    'numMCTSSims': 80,  # How many moves should the Monte Carlo Tree Search explore in every move decision.
    ...
    'checkpoint': './training_tictactoe/',  # Where to store all generated data.

    'load_model': False,  # Tells if a model will be loaded or a new one will be started.
    'load_folder_file': ('training_tictactoe',
                         'model_iteration_7.pth.tar'),  # Folder / FileName for the loaded model.
    'load_folder_file_training_data': ('training_tictactoe',
                                       'checkpoint_7.pth.tar'),  # Folder / FileName for the loaded training data.
